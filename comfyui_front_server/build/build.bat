@echo off
REM ComfyUI Front Server 编译脚本 (Windows)
REM 支持指定编译环境和打包
...
set VERSION=
set OUTPUT_DIR=dist
set PROJECT_NAME=comfyui_front_server
...
:show_help
echo ComfyUI Front Server 编译脚本
echo.
echo 用法: %~nx0 [选项]
echo.
echo 选项:
...
echo @echo off
echo chcp 65001 ^>nul
echo echo 启动 ComfyUI Front Server...
echo echo.
echo.
echo REM 检查配置文件是否存在
...
echo.
echo REM 启动服务
echo start "ComfyUI Front Server" %%~dp0%PROJECT_NAME%.exe
echo.
...
echo.
echo - `server.port` - 服务端口（默认: 8080）
echo - `database.path` - 数据库文件路径
echo - `jwt.secret` - JWT 密钥（生产环境请修改）
echo - `log.level` - 日志级别（DEBUG, INFO, WARN, ERROR）
echo.
echo ## 注意事项
echo.
echo 1. 首次运行前，请修改 `config/config.yaml` 中的敏感配置（如 JWT secret）
echo 2. 确保有写入 `data/` 目录的权限
echo 3. 生产环境建议使用反向代理（如 Nginx）
