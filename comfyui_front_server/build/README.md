# ComfyUI Front Server 构建说明

本目录包含用于编译和打包 ComfyUI Front Server 的构建脚本。
...
1. **可执行文件**: `comfyui_front_server` (Linux/macOS) 或 `comfyui_front_server.exe` (Windows)
2. **压缩包**: `comfyui_front_server-{OS}-{ARCH}-{VERSION}.tar.gz` 或 `.zip`
...
输出：
- `dist/comfyui_front_server` - 可执行文件
- `dist/comfyui_front_server-linux-amd64-1.0.0.tar.gz` - 压缩包
