...
VERSION=""
OUTPUT_DIR="dist"
PROJECT_NAME="comfyui_middleground"
PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# 显示帮助信息
show_help() {
    echo -e "${BLUE}ComfyUI Middle Server 编译脚本${NC}"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
...
REM 启动服务
start "ComfyUI Middle Server" %~dp0comfyui_middleground.exe
...
- \`data/\` - 数据目录（数据库文件和图像文件将存储在此）
  - \`data/middle.db\` - SQLite 数据库文件
  - \`data/images/\` - 图像存储目录
...
- \`server.host\` - 服务监听地址（默认: 127.0.0.1）
- \`server.port\` - 服务端口（默认: 8081）
- \`database.path\` - 数据库文件路径
- \`storage.image_path\` - 图像存储路径
...
1. 首次运行前，请修改 \`config/config.yaml\` 中的配置：
   - \`comfyui.host\` - 设置为实际的 ComfyUI 服务地址
   - \`front_server.ws_url\` - 设置为前台服务器的 WebSocket 地址
   - \`front_server.secret_key\` - 设置与前台服务器一致的密钥
2. 确保有写入 \`data/\` 目录的权限
3. 确保 ComfyUI 服务已启动并可访问
